    ...
    try {
      ...
      if (overlayUrl) {
        ...
        if (csvText) {
          ...
          // 🔥 FIXED: Apply overlay with proper HK stock handling
...
// 🔥 FIXED: Custom overlay function that prevents HK stock duplication
function applyOverlayFixed(baseData, overlayMap) {
  ...
  // 🔥 STEP 2: Add NEW tickers from overlay (but SKIP HK stocks that already exist)
  overlayMap.forEach((overlayData, ticker) => {
    if (!processedTickers.has(ticker)) {
      // This is a new ticker from overlay
      
      // 🔥 CRITICAL FIX: Skip HK stocks entirely if they haven't been processed yet
      // (They should have been processed in step 1 if they exist in base data)
      if (HK_STOCK_SYMBOLS.includes(ticker)) {
        console.log(`🚫 SKIPPING overlay HK stock ${ticker} - should be in base data or not added`)
        return // Skip all HK stocks from overlay to prevent duplication
      } else {
...
function createStockFromOverlay(overlayData) {
  ...
  return applyOverlay([newStock], new Map([[ticker, overlayData]]))[0]
}
