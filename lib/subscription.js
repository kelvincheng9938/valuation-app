...
// Add this to your lib/subscription.js file
export async function getAllSubscriptions() {
  try {
    console.log('🔍 [DEBUG] Starting getAllSubscriptions...')
    
    // Method 1: Try using scan
    const keys = [];
    let cursor = 0;
    let scanCount = 0;
    ...
    do {
      try {
        const result = await kv.scan(cursor, { match: 'subscription:*', count: 100 });
        cursor = result[0];
        keys.push(...result[1]);
        scanCount++;
        console.log(`🔍 [DEBUG] Scan ${scanCount}: Found ${result[1].length} keys, cursor: ${cursor}`);
      } catch (scanError) {
        console.error('❌ [DEBUG] Scan failed:', scanError);
        break;
      }
    } while (cursor !== 0 && scanCount < 10); // Safety limit
    
    console.log(`🔍 [DEBUG] Total keys found: ${keys.length}`, keys);
    ...
    for (const key of keys) {
      try {
        ...
        console.error(`❌ [DEBUG] Error getting ${key}:`, getError);
      }
    }
    
    console.log(`🔍 [DEBUG] Processed ${subscriptions.length} subscriptions`);
    ...
    console.error('❌ [DEBUG] Error in getAllSubscriptions:', error);
    return { 
      total: 0, 
      subscriptions: [], 
      demoUsers: DEMO_PRO_USERS,
