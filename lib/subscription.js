// lib/subscription.js - PERSISTENT DATABASE VERSION
...
export async function hasActiveSubscription(userEmail) {
  ...
  try {
    console.log(`🔍 [DB] Checking subscription for: ${userEmail}`);
    ...
    if (!subscription) {
      console.log(`❌ [DB] No subscription found for: ${userEmail}`);
      return false;
    }
    ...
    console.log(`${hasActive ? '✅' : '❌'} [DB] Subscription for ${userEmail}:`);
...
export async function updateUserSubscription(userEmail, subscriptionData) {
  try {
    console.log(`📝 [DB] Storing subscription for ${userEmail}:`, subscriptionData);
    ...
    console.log(`✅ [DB] Subscription persisted for ${userEmail}`);
    
    // Verify it was saved
    const saved = await kv.get(`subscription:${userEmail}`);
    console.log(`🔍 [DB] Verification - saved data exists: ${!!saved}`);
    
    return subscriptionRecord;
    
  } catch (error) {
    console.error('❌ [DB] Error storing subscription:', error);
    throw error;
  }
}
...
export async function getUserSubscription(userEmail) {
  ...
  try {
    ...
    const subscription = await kv.get(`subscription:${userEmail}`);
    console.log(`🔍 [DB] Retrieved subscription for ${userEmail}:`, subscription ? 'Found' : 'Not found');
...
export async function cancelUserSubscription(userEmail) {
  try {
    ...
    if (existing) {
      ...
      console.log(`✅ [DB] Subscription canceled for ${userEmail}`);
    }
...
export async function getAllSubscriptions() {
  try {
    ...
    console.error('❌ [DB] Error getting all subscriptions:', error);
