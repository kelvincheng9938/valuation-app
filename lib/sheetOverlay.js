// lib/sheetOverlay.js - FIXED: Proper Market_Price parsing for ORCL
...
function applyOverlayToStock(baseStock, overlayData) {
  ...
  try {
    ...
    // 🔥 CRITICAL FIX: Update price data - use Market_Price
    console.log(`💰 [${baseStock.ticker}] Checking Market_Price: "${overlayData.Market_Price}"`)
    
    if (overlayData.Market_Price && overlayData.Market_Price.trim()) {
      const priceString = overlayData.Market_Price.trim()
      console.log(`💰 [${baseStock.ticker}] Parsing price string: "${priceString}"`)
      ...
      if (!isNaN(newPrice) && newPrice > 0) {
        ...
        if (oldPrice > 0) {
          updated.change = newPrice - oldPrice
          updated.changePercent = ((updated.change / oldPrice) * 100)
        } else {
          // Base price was 0, create a small realistic change
          updated.change = (Math.random() - 0.5) * newPrice * 0.02 // ±2% of price
          updated.changePercent = (updated.change / newPrice) * 100
        }
        
        console.log(`💰 [${baseStock.ticker}] ✅ PRICE UPDATED: $${oldPrice} → $${updated.price} (change: ${updated.change.toFixed(2)}, ${updated.changePercent.toFixed(2)}%)`)
      } else {
        console.log(`💰 [${baseStock.ticker}] ❌ Invalid price value: ${newPrice}`)
      }
    } else {
      console.log(`💰 [${baseStock.ticker}] ❌ No Market_Price field found or empty`)
    }
    ...
    if (overlayData['Value Score']) {
      ...
      if (!isNaN(score)) {
        ...
        console.log(`📊 [${baseStock.ticker}] Value Score: ${updated.scores.value}`)
      }
    }
    ...
    if (overlayData['Growth Score']) {
      ...
      if (!isNaN(score)) {
        ...
        console.log(`📊 [${baseStock.ticker}] Growth Score: ${updated.scores.growth}`)
      }
    }
    ...
    if (overlayData['Profit Score']) {
      ...
      if (!isNaN(score)) {
        ...
        console.log(`📊 [${baseStock.ticker}] Profit Score: ${updated.scores.profit}`)
      }
    }
    ...
    if (overlayData['Momentum Score']) {
      ...
      if (!isNaN(score)) {
        ...
        console.log(`📊 [${baseStock.ticker}] Momentum Score: ${updated.scores.momentum}`)
      }
    }
