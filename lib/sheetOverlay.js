// lib/sheetOverlay.js - FIXED: Include main ticker in peers + bigger dots
...
function applyOverlayToStock(baseStock, overlayData) {
  ...
  try {
    ...
    const mainTicker = overlayData.Ticker?.replace(/\s+US\s+EQUITY$/i, '').toUpperCase()
    ...
    // 🔥 FIXED: Update segments with new percentage structure
    const segments = []
    const segmentColors = ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#06b6d4']
    
    // Get all segment and percentage columns
    const segmentKeys = Object.keys(overlayData).filter(key => key.startsWith('Segment '))
    const percentageKeys = Object.keys(overlayData).filter(key => key.includes('%'))
    
    console.log(`🔍 Found segment keys for ${baseStock.ticker}:`, segmentKeys)
    console.log(`🔍 Found percentage keys for ${baseStock.ticker}:`, percentageKeys)
    
    // Handle segments - pair them with their corresponding percentages
    for (let i = 1; i <= 6; i++) {
      const segmentName = overlayData[`Segment ${i}`]
      
      // Try different percentage column formats that might exist
      let segmentPercent = null
      
      // Look for the percentage in various possible column names
      const possiblePercentCols = [
        `${i * 10}%`,           // Like "10%", "20%", etc.
        `0.${i}0%`,             // Like "0.10%", "0.20%", etc.
        `%`,                    // Single % column (less likely to work well)
        `Segment ${i} %`,       // Explicit naming
      ]
      
      for (const colName of possiblePercentCols) {
        if (overlayData[colName] && overlayData[colName].trim()) {
          segmentPercent = overlayData[colName]
          break
        }
      }
      
      if (segmentName && segmentName.trim() && segmentPercent) {
        const percentNum = parseFloat(segmentPercent.replace('%', ''))
        if (!isNaN(percentNum) && percentNum > 0) {
          ...
          console.log(`✅ Added segment: ${segmentName.trim()} = ${percentNum}%`)
        }
      }
    }
